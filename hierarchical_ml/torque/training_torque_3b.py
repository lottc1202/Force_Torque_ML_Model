...
def saving_models(eph,fold_ind,mdl,optmzr,schdlr,mdl_type='_torque_3b'):
...
#global plotter
#plotter = utils.VisdomLinePlotter(env_name='torque',prt=str(opt.prt))
...
for fld_i in range(opt.k_fold):
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i,datas in enumerate(dataloader):
            ...
            real = real - (mdl_2b(vectors_1b,invar_2b,
                           vectors_2b).detach())
            wgt = datas['wgt_t'].to(device)
            """
            if (i % 10 == 0) :
                print(fake[0,:])
                print(real[0,:])
        ...
        if (cur_val>=prev_val) and (prev_val > mean_scheduler):
            ...
            if (val_cnt >= 50) and (epoch > 99):
                saving_models(epoch+1,fld_i,mdl_3b,optimizer_3b,
                              scheduler_3b,mdl_type='_torque_3b')
                break

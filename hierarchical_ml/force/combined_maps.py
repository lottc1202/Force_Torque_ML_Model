...
for fld_i in range(opt.k_fold):
    ...
    load_path3b = load_path3b + "_l_"+str(10)+"_w_"+str(50)
    ...
    with torch.no_grad():
        for i in range(0,xyz.size(dim=0),100):
            ...
            fake = (mdl_3b(invar_3b_1,vectors_3b)+
                    mdl_2b(invar_2b_1,vectors_2b_1)+
                    mdl_2b(invar_2b_1,vectors_2b_2))
...
thisdict = {'fake':fk_np,'xyz':xyz_np,
            'n_xyz':n_xyz_np,
            'inpts':invar_np}
sio.savemat("combined_inflc_map_"+opt.fl_tag
             +".mat",thisdict)

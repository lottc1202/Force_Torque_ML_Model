...
parser.add_argument("--num_neig",type=int,default=5,help="Number of sorted neighbors to consider")
...
fl_lst = list(range(1,18))

# Removing dataset 9 as it has faulty data
fl_lst.remove(9)
...
def saving_models(eph,fold_ind,mdl,optmzr,schdlr,mdl_type='_force_3b'):
...
# Loss function
criterion_loss = torch.nn.MSELoss(reduction='mean')
...
for fld_i in range(opt.k_fold):
    ...
    mdl_2b = frc_2b(lyr_wdt=50,
                    num_lyrs=6,
                    act1=torch.nn.ReLU(),
                    act2=torch.tanh)

    # Load pre-trained frc_2b model
    ...
    load_path2b = load_path2b + "_l_"+str(6)+"_w_"+str(50)
    ...
    optimizer_3b = torch.optim.Adam(mdl_3b.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
    scheduler_3b = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_3b,patience=10,factor=0.1)
    mdl_3b.to(device)
    ...
    if torch.cuda.device_count() >1:
        ...
        mdl_2b.to(device)
        mdl_3b = torch.nn.DataParallel(mdl_3b)
        mdl_3b.to(device)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i,datas in enumerate(dataloader):
            ...
            real = real - (frc_1b(invar_1b,vectors_1b)+
                    mdl_2b(invar_2b,vectors_2b).detach())
            ...
            """
            # Loss
            loss_3b = criterion_loss(w_lft*wgt*fake,w_lft*wgt*real)
            optimizer_3b.zero_grad()  # Clearing out previous gradients
            loss_3b.backward()
            optimizer_3b.step()
            ...
            if (batches_done % 10 == 0):
                grad_track = []
                for p in mdl_3b.parameters():
                    grad_track.append(p.grad.abs().mean().unsqueeze(0))
                grad_track = torch.cat(grad_track,dim=0)
                print(grad_track.mean())
            """
            # Learning Rate scheduling metric
            mean_scheduler = mean_scheduler + loss_3b.item()
        ...
        with torch.no_grad():
            for i,datas in enumerate(val_dataloader):
                ...
                loss_val = criterion_loss(w_lft*wgt*fake,w_lft*wgt*real)
        ...
        """
        if (epoch % 2 == 0):
            plotter.plot('3b_loss_epoch'+str(fld_i),'Training','ThreeBody',epoch+1,mean_scheduler)
            plotter.plot('3b_loss_epoch'+str(fld_i),'Validation','ThreeBody',epoch+1,cur_val.item())
        """
        scheduler_3b.step(mean_scheduler) # Scheduler to reduce learning rate
        ...
        if (cur_val>=prev_val) and (prev_val > mean_scheduler):
            ...
            if (val_cnt >= 50) and (epoch > 99):
                saving_models(epoch+1,fld_i,mdl_3b,optimizer_3b,
                              scheduler_3b,mdl_type='_force_3b')
                break
    ...
    sio.savemat("loss_evolution/bx_sz_LN_26_neig_"+
                 ...
                 str(opt.k_fold)+"_"+str(fld_i)+
                 "_l_"+str(opt.num_lyrs)+"_w_"+
                 str(opt.lyr_wdt)+"_3b.mat",thisdict)
